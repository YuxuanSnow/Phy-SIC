...
class DatasetVal(Dataset):
    def __init__(self, cfg, dataset, is_train=False):
        ...
        self.normalize_img = Normalize(
            mean=cfg.MODEL.IMAGE_MEAN, std=cfg.MODEL.IMAGE_STD
        )
        ...
        if "pose_cam" in self.data:
            if "smplx" in self.dataset:
                self.pose = self.data["pose_cam"][:, : NUM_PARAMS_SMPLX * 3].astype(
                    np.float
                )
            else:
                self.pose = self.data["pose_cam"][:, : NUM_PARAMS_SMPL * 3].astype(
                    np.float
                )
        else:
        ...
        if "part" in self.data:
            ...
            self.keypoints = self.data["gtkps"][
                :, :NUM_JOINTS
            ]  # Todo later: change it to a variable
        ...
        try:
            ...
            self.gender = np.array(
                [0 if str(g) == "m" or str(g) == "male" else 1 for g in gender]
            ).astype(np.int32)
        except KeyError:
    ...
    def __getitem__(self, index):
        ...
        item["img_full_resized"] = self.normalize_img(
        ...
        item["cam_int"] = np.array(
            [[fl, 0, img_w / 2.0], [0, fl, img_h / 2.0], [0, 0, 1]]
        ).astype(np.float32)
        ...
        if "smplx" in self.dataset:
            ...
            gt_smpl_out = model(
                global_orient=torch.from_numpy(
                    item["smpl_params"]["global_orient"]
                ).unsqueeze(0),
                body_pose=torch.from_numpy(item["smpl_params"]["body_pose"]).unsqueeze(
                    0
                ),
            ...
            item["keypoints_3d"] = torch.matmul(
                self.smpl_gt_neutral.J_regressor, gt_vertices[0]
            )
