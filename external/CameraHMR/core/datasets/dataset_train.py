...
class DatasetTrain(Dataset):
    def __init__(self, cfg, dataset, is_train=True):
        ...
        if "pose_cam" in self.data:
            self.body_pose = self.data["pose_cam"][:, : NUM_PARAMS_SMPL * 3].astype(
                np.float
            )  # Change 24
        elif "pose" in self.data:
            self.body_pose = self.data["pose"][:, : NUM_PARAMS_SMPL * 3].astype(
                np.float
            )  # Change 24
        else:
            self.body_pose = np.zeros(
                (len(self.imgname), NUM_PARAMS_SMPL * 3), dtype=np.float32
            )
    ...
    def __getitem__(self, index):
        ...
        smpl_params = {
            "global_orient": self.body_pose[index][:3].astype(np.float32),
