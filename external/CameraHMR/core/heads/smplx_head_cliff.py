...
class SMPLXTransformerDecoderHead(nn.Module):
    def __init__(self):
        ...
        init_body_pose = (
            torch.eye(3)
            .reshape(1, 3, 3)
            .repeat(NUM_POSE_PARAMS + 1, 1, 1)[:, :, :2]
            .flatten(1)
            .reshape(1, -1)
        )
        init_body_pose[:, : 22 * 6] = torch.from_numpy(
            mean_params["pose"][: 22 * 6]
        ).float()  # global_orient+body_pose from SMPL

        init_betas = torch.zeros((1, NUM_BETAS)).float()
        init_betas[:, :10] = torch.from_numpy(
    ...
    def forward(self, x, bbox_info, **kwargs):
        ...
        pred_body_pose = joint_conversion_fn(pred_body_pose).view(
            batch_size, NUM_POSE_PARAMS + 1, 3, 3
        )
