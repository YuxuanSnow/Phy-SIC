...
class CameraHMR(pl.LightningModule):
    ...
    def __init__(self, cfg: CfgNode):
        ...
        self.backbone.load_state_dict(
            torch.load(VITPOSE_BACKBONE, map_location="cpu")["state_dict"]
        )
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        pred_smpl_params, pred_cam, _, pred_kp = self.smpl_head(
        ...
        if train:
            smpl_output_gt = self.smpl(
                **{k: v.float() for k, v in batch["smpl_params"].items()}
            )
            batch["gt_vertices"] = smpl_output_gt.vertices
            ones = torch.ones((batch_size, NUM_JOINTS, 1), device=self.device)
            batch["keypoints_3d"] = torch.cat(
                (smpl_output_gt.joints[:, :NUM_JOINTS], ones), dim=-1
            )
        ...
        if (
            self.cfg.LOSS_WEIGHTS["VERTS2D"]
            or self.cfg.LOSS_WEIGHTS["VERTS2D_CROP"]
            or self.cfg.LOSS_WEIGHTS["VERTS_2D_NORM"]
        ):
    ...
    def perspective_projection_vis(self, input_batch, output, max_save_img=1):
        ...
        for i in range(len(input_batch["imgname"])):
            ...
            save_filename = os.path.join(
                ".", f"{self.global_step:08d}_{i:02d}_{os.path.basename(imgname)}"
            )
    ...
    def compute_loss(
        ...
        loss_vertices = self.vertices_loss(
            output["pred_vertices"], batch["gt_vertices"]
        )
        ...
        for k, pred in pred_smpl_params.items():
            ...
            loss_smpl_params[k] = self.smpl_parameter_loss(
                pred.reshape(batch_size, -1), gt.reshape(batch_size, -1)
            )
        ...
        if self.cfg.LOSS_WEIGHTS["VERTS2D_CROP"]:
            ...
            pred_verts2d_cropped = trans_points2d_parallel(
                pred_verts2d, batch["_trans"]
            )
            pred_verts2d_cropped = (
                pred_verts2d_cropped / self.cfg.MODEL.IMAGE_SIZE - 0.5
            )
            gt_verts_2d_cropped = gt_verts2d.clone()
            gt_verts_2d_cropped[:, :, :2] = trans_points2d_parallel(
                gt_verts2d[:, :, :2], batch["_trans"]
            )
            gt_verts_2d_cropped[:, :, :2] = (
                gt_verts_2d_cropped[:, :, :2] / self.cfg.MODEL.IMAGE_SIZE - 0.5
            )

            loss_proj_vertices_cropped = self.keypoint_2d_loss(
                pred_verts2d_cropped, gt_verts_2d_cropped
            )
        ...
        if self.cfg.LOSS_WEIGHTS["VERTS_2D_NORM"]:
            ...
            pred_verts2d[:, :, :2] = (
                pred_verts2d[:, :, :2] - pred_verts2d[:, [0], :2]
            ) / batch["box_size"].unsqueeze(-1).unsqueeze(-1)
            gt_verts2d[:, :, :2] = (
                gt_verts2d[:, :, :2] - gt_verts2d[:, [0], :2]
            ) / batch["box_size"].unsqueeze(-1).unsqueeze(-1)
        ...
        if self.cfg.LOSS_WEIGHTS["KEYPOINTS_2D_CROP"]:
            ...
            pred_keypoints_2d_cropped = trans_points2d_parallel(
                pred_keypoints_2d, batch["_trans"]
            )
            pred_keypoints_2d_cropped = (
                pred_keypoints_2d_cropped / self.cfg.MODEL.IMAGE_SIZE - 0.5
            )

            loss_keypoints_2d_cropped = self.keypoint_2d_loss(
                pred_keypoints_2d_cropped, gt_keypoints_2d
            )
            loss += (
                self.cfg.LOSS_WEIGHTS["KEYPOINTS_2D_CROP"] * loss_keypoints_2d_cropped
            )

        if self.cfg.LOSS_WEIGHTS["KEYPOINTS_2D"]:
            pred_keypoints_2d_clone = pred_keypoints_2d.clone()
            pred_keypoints_2d_clone[:, :, :2] = (
                2 * (pred_keypoints_2d_clone[:, :, :2] / img_size) - 1
            )
            gt_keypoints_2d = batch["orig_keypoints_2d"]
            gt_keypoints_2d[:, :, :2] = 2 * (gt_keypoints_2d[:, :, :2] / img_size) - 1
            loss_keypoints_2d = self.keypoint_2d_loss_scaled(
                pred_keypoints_2d_clone, gt_keypoints_2d, batch["box_size"], img_size
            )
        ...
        if self.cfg.LOSS_WEIGHTS["KEYPOINTS_2D_NORM"]:
            ...
            pred_keypoints_2d_clone = pred_keypoints_2d.clone()
            pred_keypoints_2d_clone[:, :, :2] = (
                pred_keypoints_2d_clone[:, :, :2] - pred_keypoints_2d_clone[:, [0], :2]
            ) / batch["box_size"].unsqueeze(-1).unsqueeze(-1)
            gt_keypoints_2d = batch["orig_keypoints_2d"].clone()
            gt_keypoints_2d[:, :, :2] = (
                gt_keypoints_2d[:, :, :2] - gt_keypoints_2d[:, [0], :2]
            ) / batch["box_size"].unsqueeze(-1).unsqueeze(-1)
            loss_keypoints_2d_norm = self.keypoint_2d_loss(
                pred_keypoints_2d_clone, gt_keypoints_2d
            )
    ...
    def validation_step(self, batch: Dict, batch_idx: int, dataloader_idx=0) -> Dict:
        ...
        J_regressor_batch_smpl = (
            self.J_regressor[None, :]
            .expand(batch["img"].shape[0], -1, -1)
            .float()
            .cuda()
        )
        ...
        if "3dpw" in dataset_names[0]:
            ...
            pred_pelvis = (
                pred_keypoints_3d[:, [1], :] + pred_keypoints_3d[:, [2], :]
            ) / 2.0
            pred_keypoints_3d = pred_keypoints_3d - pred_pelvis
            pred_cam_vertices = pred_cam_vertices - pred_pelvis
            ...
            female_batch = {
                k: v[female_indices] for k, v in batch["smpl_params"].items()
            }

            # Create an empty tensor with the same shape as the original batch
            output_shape = (
                batch["gender"].shape[0],
                6890,
                3,
            )  # Assuming the output shape is the same for both models
            smpl_output_gt = torch.empty(
                output_shape,
                dtype=self.smpl_gt().vertices.dtype,
                device=batch["gender"].device,
            )
            ...
            if female_indices.any():
                smpl_output_gt[female_indices] = self.smpl_gt_female(
                    **female_batch
                ).vertices
        ...
        if batch["keypoints_2d"].shape[1] >= 17:
            ...
            if "3dpw" in dataset_names[0]:
                gt_kp = torch.cat(
                    (gt_kp[:, :9, :], zeros_to_insert, gt_kp[:, 9:, :]), dim=1
                )
                pck1, avgpck1, _ = pck_accuracy(
                    pred_kp[:, :18, :2], gt_kp[:, :18, :2], mask[:, :18], 0.05
                )
                pck2, avgpck2, _ = pck_accuracy(
                    pred_kp[:, :18, :2], gt_kp[:, :18, :2], mask[:, :18], 0.1
                )
            else:
                pck1, avgpck1, _ = pck_accuracy(
                    pred_kp[:, :18, :2], gt_kp[:, :18, :2], mask[:, :18], 0.05
        ...
        error_verts = torch.sqrt(
            ((pred_cam_vertices - gt_cam_vertices) ** 2).sum(dim=-1)
        )
        ...
        self.validation_step_output.append(
            {
                "val_loss": val_pve,
                "val_loss_mpjpe": val_mpjpe,
                "val_loss_pampjpe": val_pampjpe,
                "avgpck_0.05": avgpck_005,
                "avgpck_0.1": avgpck_01,
    ...
    def on_validation_epoch_end(self, dataloader_idx=0):
        ...
        for dataloader_idx in range(len(val_dataset)):
            dataloader_outputs = [
                x for x in outputs if x.get("dataloader_idx") == dataloader_idx
            ]
            if dataloader_outputs:  # Ensure there are outputs for this dataloader
                avg_val_loss = torch.stack(
                    [x["val_loss"] for x in dataloader_outputs]
                ).mean()
                avg_mpjpe_loss = torch.stack(
                    [x["val_loss_mpjpe"] for x in dataloader_outputs]
                ).mean()
                avg_pampjpe_loss = torch.stack(
                    [x["val_loss_pampjpe"] for x in dataloader_outputs]
                ).mean()

                avg_pck_005_loss = torch.stack(
                    [x["avgpck_0.05"] for x in dataloader_outputs]
                ).mean()
                avg_pck_01_loss = torch.stack(
                    [x["avgpck_0.1"] for x in dataloader_outputs]
                ).mean()
                ...
                logger.info(
                    "avgpck_0.05: " + str(dataloader_idx) + str(avg_pck_005_loss)
                )
