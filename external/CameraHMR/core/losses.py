...
class Keypoint2DLoss(nn.Module):
    ...
    def forward(
        ...
        loss = (
            conf * self.loss_fn(pred_keypoints_2d.float(), gt_keypoints_2d[:, :, :-1])
        ).sum(dim=(1, 2))
        return loss.sum()
...
class Keypoint2DLossScaled(nn.Module):
    ...
    def forward(
        ...
        loss = conf * self.loss_fn(
            pred_keypoints_2d.float(), gt_keypoints_2d[:, :, :-1]
        )
...
class Keypoint3DLoss(nn.Module):
    ...
    def forward(
        ...
        loss = (conf * self.loss_fn(pred_keypoints_3d.float(), gt_keypoints_3d)).sum(
            dim=(1, 2)
        )
        return loss.sum()
