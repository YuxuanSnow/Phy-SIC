...
def covert_keypoint_definition(keypoints, pose_det_dataset, pose_lift_dataset):
    ...
    """
    if (
        pose_det_dataset == "TopDownH36MDataset"
        and pose_lift_dataset == "Body3DH36MDataset"
    ):
        return keypoints
    elif (
...
def main():
    ...
    assert pose_det_model.cfg.model.type == "TopDown", (
        ...
        if save_out_video:
            if writer is None:
                writer = cv2.VideoWriter(
                    osp.join(
                        args.out_video_root, f"vis_{osp.basename(args.video_path)}"
                    ),
                    fourcc,
                    fps,
