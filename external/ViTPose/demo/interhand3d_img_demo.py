...
def main():
    ...
    parser.add_argument(
        "--json-file", type=str, default="", help="Json file containing image info."
    )
    parser.add_argument(
        "--camera-param-file",
        type=str,
    ...
    for i, det_results in enumerate(mmcv.track_iter_progress(det_results_list)):
        ...
        for idx, res in enumerate(pose_results):
            ...
            if camera_param is not None and keypoints_3d_gt is not None:
                ...
                keypoints_3d_gt[:, :3] = camera.world_to_camera(keypoints_3d_gt[:, :3])
                ...
                keypoints_3d[:, :3] = camera.pixel_to_camera(keypoints_3d[:, :3])
            ...
            if args.show_ground_truth:
                if keypoints_3d_gt is None:
                    print(
                        "Fail to show ground truth. Please make sure that"
