...
def parse_args():
    ...
    parser.add_argument(
        "--det-config",
        type=str,
        default="demo/mmdetection_cfg/ssdlite_mobilenetv2_scratch_600e_coco.py",
    ...
    parser.add_argument(
        "--enable-human-pose", type=int, default=1, help="Enable human pose estimation"
    )
    parser.add_argument(
        "--enable-animal-pose",
        type=int,
    ...
    parser.add_argument(
        "--det-score-thr", type=float, default=0.5, help="bbox score threshold"
    )
    parser.add_argument(
    ...
    parser.add_argument("--bugeye", action="store_true", help="Apply `bug-eye` effect.")
...
def display():
    ...
    fps_inference = 0.0  # infenrece FPS
    t_delay_inference = 0.0  # inference result time delay
    ...
    while True:
        with stop_watch.timeit("_FPS_"):
                ...
                for model_info, pose_results in zip(pose_model_list, pose_results_list):
                    ...
                    if args.sunglasses:
                        if dataset_name in {
                            ...
                            raise ValueError(
                                f"Sunglasses effect does not support{dataset_name}"
                            )
                    ...
                    if args.bugeye:
                        if dataset_name in {
                            ...
                            raise ValueError(
                                f"Bug-eye effect does not support{dataset_name}"
                            )
                        img = apply_bugeye_effect(
                            img, pose_results, left_eye_idx, right_eye_idx
                        )
            ...
            cv2.putText(
                img, t_info_str, (20, 20), cv2.FONT_HERSHEY_DUPLEX, 0.3, text_color, 1
            )
            ...
            cv2.putText(
                img, sys_info_str, (20, 40), cv2.FONT_HERSHEY_DUPLEX, 0.3, text_color, 1
            )
            ...
            if args.out_video_file is not None:
                if vid_out is None:
                    ...
                    vid_out = cv2.VideoWriter(
                        args.out_video_file, fourcc, fps, frame_size
                    )
...
def main():
    ...
    if args.buffer_size > 0:
        ...
        buffer_size = round(30 * (1 + max(args.display_delay, 0) / 1000.0))
