...
for subtopic, topic, task in sorted(subtopics):
    ...
    datasets = sorted(
        list(set(x.split("/")[-2] for x in glob(f"../../configs/{topic}/{task}/*/*/")))
    )
    ...
    for dataset in datasets:
        ...
        for file in glob(f"../../configs/{topic}/{task}/*/{dataset}/*.md"):
            keywords = (file.split("/")[-3], *file.split("/")[-1].split("_")[:-1])
    ...
    for dataset, keywords in datasets.items():
        ...
        for keyword, info in keywords.items():
            ...
            lines += [
                "<br/>",
                "",
                (f"### {' + '.join(keyword_strs)} on {titlecase(dataset)}"),
                "",
                info,
                "",
            ]
...
for section in sections:
    ...
    for file in files:
        with open(f"../en/papers/{section}/{file}", "r") as f:
            keyline = [line for line in f.readlines() if line.startswith("<summary")][0]
