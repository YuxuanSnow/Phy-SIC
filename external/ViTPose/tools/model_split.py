...
def main():
    ...
    for i in range(5):
        ...
        for tensor_name in weight_names:
            new_ckpt["state_dict"][tensor_name] = new_ckpt["state_dict"][
        ...
        for tensor_name in [
            ...
            new_ckpt["state_dict"][tensor_name] = new_ckpt["state_dict"][tensor_name][
                : num_keypoints[i]
            ]
        ...
        for j in range(5):
            ...
            for tensor_name in weight_names:
                new_ckpt["state_dict"].pop(
                    tensor_name.replace(
                        "keypoint_head", f"associate_keypoint_heads.{j}"
                    )
                )
        # remove expert part
