...
class BufferManager:
    def __init__(self, buffer_type: type = Buffer, buffers: Optional[Dict] = None):
        ...
        if buffers is None:
            ...
            if is_seq_of(list(buffers.values()), buffer_type):
                ...
                raise ValueError(
                    f"The values of buffers should be instance of {buffer_type}"
                )
    ...
    def get_info(self):
        ...
        for name, buffer in self._buffers.items():
            buffer_info[name] = {"size": buffer.size, "maxsize": buffer.maxsize}
        return buffer_info
