...
def get_eye_keypoint_ids(model_cfg: Config) -> Tuple[int, int]:
    ...
    if left_eye_idx is None or right_eye_idx is None:
        ...
        if dataset_name in {"TopDownCocoDataset", "TopDownCocoWholeBodyDataset"}:
            left_eye_idx = 1
            right_eye_idx = 2
            ...
            raise ValueError(f"Can not determine the eye keypoint id of {dataset_name}")
...
def get_face_keypoint_ids(model_cfg: Config) -> Tuple[int, int]:
    ...
    try:
        dataset_info = DatasetInfo(model_cfg.data.test.dataset_info)
        for id in range(68):
            face_indices.append(dataset_info.keypoint_name2id.get(f"face_{id}", None))
    except AttributeError:
        face_indices = None
    ...
    if face_indices is None:
        ...
        if dataset_name in {"TopDownCocoWholeBodyDataset"}:
            ...
            raise ValueError(f"Can not determine the face id of {dataset_name}")
...
def get_hand_keypoint_ids(model_cfg: Config) -> List[int]:
    ...
    try:
        ...
        hand_indices.append(dataset_info.keypoint_name2id.get("left_hand_root", None))
        ...
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"left_forefinger{id}", None)
            )
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"left_middle_finger{id}", None)
            )
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"left_ring_finger{id}", None)
            )
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"left_pinky_finger{id}", None)
            )

        hand_indices.append(dataset_info.keypoint_name2id.get("right_hand_root", None))
        ...
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"right_forefinger{id}", None)
            )
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"right_middle_finger{id}", None)
            )
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"right_ring_finger{id}", None)
            )
        for id in range(1, 5):
            hand_indices.append(
                dataset_info.keypoint_name2id.get(f"right_pinky_finger{id}", None)
            )
    ...
    if hand_indices is None:
        ...
        if dataset_name in {"TopDownCocoWholeBodyDataset"}:
            ...
            raise ValueError(f"Can not determine the hand id of {dataset_name}")
