    ...
    def update_route_info(
        ...
        if node is not None:
            if node_name is not None or node_type is not None:
                warnings.warn(
                    "`node_name` and `node_type` will be overridden if nodeis provided."
                )
            node_name = node.name
...
class FrameMessage(Message):
    ...
    def get_detection_results(self, tag: str = None):
        ...
        if tag is None:
            ...
            results = [
                res for _tag, res in self.data["detection_results"] if _tag == tag
            ]
        return results
    ...
    def get_pose_results(self, tag=None):
        ...
        if tag is None:
            ...
            results = [res for _tag, res in self.data["pose_results"] if _tag == tag]
        return results
