    ...
    def draw(self, frame_msg):
        ...
        for pose_result in frame_msg.get_pose_results():
            ...
            if isinstance(self.bbox_color, dict):
                bbox_colors = [
                    self.bbox_color.get(label, (0, 255, 0)) for label in bbox_labels
                ]
            else:
...
class SunglassesNode(FrameDrawingNode):
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = apply_sunglasses_effect(
                canvas, preds, self.src_img, left_eye_idx, right_eye_idx
            )
        return canvas
...
class SpriteNode(FrameDrawingNode):
    def __init__(
        ...
        self.src_img = load_image_from_disk_or_url(src_img_path, cv2.IMREAD_UNCHANGED)[
            :144, :108
        ]
    ...
    def apply_sprite_effect(
        self, img, pose_results, left_hand_index, right_hand_index, kpt_thr=0.5
    ):
        """Apply sprite effect.
        ...
        if kpts[left_hand_index, 2] < kpt_thr and kpts[right_hand_index, 2] < kpt_thr:
            aim = self.pos
        else:
        ...
        img[x : x + hm, y : y + wm] = (
            img[x : x + hm, y : y + wm] * (1 - sprite[:, :, 3:] / 255)
            + sprite[:, :, :3] * (sprite[:, :, 3:] / 255)
        ).astype("uint8")
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = self.apply_sprite_effect(
                canvas, preds, left_hand_idx, right_hand_idx
            )
        return canvas
...
class BackgroundNode(FrameDrawingNode):
    ...
    def apply_background_effect(
        self, img, det_results, background_img, effect_region=(0.2, 0.2, 0.8, 0.8)
    ):
        """Change background.
    ...
    def draw(self, frame_msg):
        ...
        for det_result in det_results:
            ...
            if self.cls_ids:
                ...
                filtered_preds = [p for p in preds if p["cls_id"] in self.cls_ids]
            elif self.cls_names:
                # Filter results by class name
                filtered_preds = [p for p in preds if p["label"] in self.cls_names]
            else:
...
class SaiyanNode(FrameDrawingNode):
    ...
    def apply_saiyan_effect(
        ...
        for pose in pose_results:
            ...
            kpts = pose["keypoints"]
            if (
                kpts[face_indices[0], 2] < kpt_thr
                or kpts[face_indices[16], 2] < kpt_thr
            ):
                continue
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = self.apply_saiyan_effect(
                canvas, preds, self.hair_img, frame, face_indices
            )
...
class MoustacheNode(FrameDrawingNode):
    ...
    def apply_moustache_effect(
        self, img, pose_results, moustache_img, face_indices, kpt_thr=0.5
    ):
        ...
        pts_src = np.array(
            [[1164, 741], [1729, 741], [1164, 1244], [1729, 1244]], dtype=np.float32
        )

        for pose in pose_results:
            kpts = pose["keypoints"]
            if (
                kpts[face_indices[32], 2] < kpt_thr
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            preds = pose_result["preds"]
            face_indices = get_face_keypoint_ids(model_cfg)
            canvas = self.apply_moustache_effect(
                canvas, preds, self.src_img, face_indices
            )
        return canvas
...
class BugEyeNode(FrameDrawingNode):
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = apply_bugeye_effect(canvas, preds, left_eye_idx, right_eye_idx)
        return canvas
...
class HatNode(FrameDrawingNode):
    ...
    @staticmethod
    def apply_hat_effect(
        img, pose_results, hat_img, left_eye_index, right_eye_index, kpt_thr=0.5
    ):
        """Apply hat effect.
        Args:
            img (np.ndarray): Image data.
        ...
        b = 0.7
        pts_src = np.array(
            [[a * wm, a * hm], [a * wm, b * hm], [b * wm, a * hm], [b * wm, b * hm]],
            dtype=np.float32,
        )
        ...
        for pose in pose_results:
            ...
            if kpts[left_eye_index, 2] < kpt_thr or kpts[right_eye_index, 2] < kpt_thr:
                continue
            ...
            pts_tar = np.vstack(
                [
                    kpt_reye + 1 * veye + 5 * vo,
                    kpt_reye + 1 * veye + 1 * vo,
                    kpt_leye - 1 * veye + 5 * vo,
                    kpt_leye - 1 * veye + 1 * vo,
                ]
            )
...
class FirecrackerNode(FrameDrawingNode):
    ...
    @staticmethod
    def apply_firecracker_effect(
        img, pose_results, firecracker_img, left_wrist_idx, right_wrist_idx, kpt_thr=0.5
    ):
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = self.apply_firecracker_effect(
                canvas, preds, frame, left_wrist_idx, right_wrist_idx
            )
        self.frame_idx = (self.frame_idx + 1) % (self.num_frames * self.frame_period)
