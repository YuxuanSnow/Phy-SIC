...
class ValentineMagicNode(FrameDrawingNode):
    def __init__(
        ...
        super().__init__(name, frame_buffer, output_buffer, enable_key=enable_key)
    ...
    def _check_heart(
        ...
        if both_hands_vis:
            ...
            hand_dis = self._cal_distance(
                kpts[hand_indices[20]][:2], kpts[hand_indices[41]][:2]
            )

            if (
                left_angle < self.hand_heart_angle_thr
        ...
        left_mid_hand_pos = (kpts[hand_indices[9]][:2] + kpts[hand_indices[12]][:2]) / 2
        ...
        right_mid_hand_pos = (
            kpts[hand_indices[30]][:2] + kpts[hand_indices[33]][:2]
        ) / 2
    ...
    def _get_heart_route(
        self,
        heart_type: int,
        cur_pred: Dict[str, np.ndarray],
        ...
        assert heart_type in [1, 2, 3], "Can not determine the type of heart effect"
    ...
    def _draw_heart(
        self, canvas: np.ndarray, heart_info: HeartInfo, t_pass: float
    ) -> np.ndarray:
        ...
        assert heart_info.heart_type in [1, 2, 3], (
            ...
            for i in range(self.num_persons):
                ...
                if id in self.heart_infos.copy():
                    ...
                    if t_pass < self.longest_duration:
                        canvas = self._draw_heart(canvas, self.heart_infos[id], t_pass)
                    ...
                    heart_type = self._check_heart(
                        preds[i], hand_indices, mouth_index, eye_indices
                    )
