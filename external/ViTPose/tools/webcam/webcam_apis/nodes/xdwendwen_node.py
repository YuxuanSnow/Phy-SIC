...
class XDwenDwenNode(FrameDrawingNode):
    ...
    def _init_resource(self, resource_file):
        ...
        for ann in anns:
            ...
            keypoints = sorted(rst["step_1"]["result"], key=lambda x: x["order"])
            ...
            mask_pts = np.array(
                [[pt["x"], pt["y"]] for pt in rst["step_2"]["result"][0]["pointList"]]
            )
            ...
            img = cv2.resize(
                img, dsize=None, fx=w_scale, fy=h_scale, interpolation=cv2.INTER_CUBIC
            )
    ...
    def _reset_dynamic(self):
        ...
        x_step = (x_tar - self.dynamic_info.pos_curr[0]) / self.dynamic_max_step
        ...
        if full_pose_results:
            for pose_results in full_pose_results:
                ...
                pred["dataset"] = DatasetInfo(
                    pose_results["model_cfg"].data.test.dataset_info
                ).dataset_name
    ...
    def _get_transform(self, kpts_src, kpts_tar):
        if self.rigid:
            ...
            trans_mat = np.array(
                [[M[0], M[1], M[2]], [-M[1], M[0], M[3]]], dtype=np.float32
            )
            ...
            if (
                np.linalg.norm(kpts_tar[0] - kpts_tar[2])
                - np.linalg.norm(kpts_tar[1] - kpts_tar[2])
            ) * (
                np.linalg.norm(kpts_src[0] - kpts_src[2])
                - np.linalg.norm(kpts_src[1] - kpts_src[2])
            ) < 0:
