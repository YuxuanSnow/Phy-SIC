...
class Node(Thread, metaclass=ABCMeta):
    ...
    def register_input_buffer(
        self, buffer_name: str, input_name: str, essential: bool = False
    ):
    ...
    def set_runner(self, runner):
        ...
        buffer_names = [
            buffer.buffer_name for buffer in self._input_buffers + self._output_buffers
        ]
        self._buffer_manager = runner.buffer_manager.get_sub_manager(buffer_names)
    ...
    def _get_input_from_buffer(self) -> Tuple[bool, Optional[Dict]]:
        ...
        # Default input
        result = {buffer_info.input_name: None for buffer_info in self._input_buffers}
