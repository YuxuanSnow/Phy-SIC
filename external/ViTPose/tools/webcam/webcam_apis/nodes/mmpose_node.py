...
from mmpose.apis import get_track_id, inference_top_down_pose_model, init_pose_model
...
class TopDownPoseEstimatorNode(Node):
    def __init__(
        ...
        self.model = init_pose_model(
            self.model_config, self.model_checkpoint, device=self.device.lower()
        )

        # Store history for pose tracking
        self.track_info = {"next_id": 0, "last_pose_preds": [], "last_time": None}
    ...
    def process(self, input_msgs: Dict[str, Message]) -> Message:
        ...
        for det_result in det_results:
            ...
            if self.cls_ids:
                ...
                det_preds = [p for p in det_preds if p["cls_id"] in self.cls_ids]
            elif self.cls_names:
                # Filter detection results by class name
                det_preds = [p for p in det_preds if p["label"] in self.cls_names]
            full_det_preds.extend(det_preds)

        # Inference pose
        pose_preds, _ = inference_top_down_pose_model(
            self.model, img, full_det_preds, bbox_thr=self.bbox_thr, format="xyxy"
        )
