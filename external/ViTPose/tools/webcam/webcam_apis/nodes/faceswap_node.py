...
class FaceSwapNode(FrameDrawingNode):
    ...
    def draw(self, frame_msg):
        ...
        cv2.putText(
            canvas, mode_txt, (10, 50), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 126, 0), 1
        )
    ...
    def _crop_face_by_contour(self, img, contour):
        ...
        mask = cv2.dilate(mask, kernel=np.ones((9, 9), dtype=np.uint8), anchor=(4, 0))
    ...
    def _swap_face(self, img_src, img_tar, face_src, face_tar):
        ...
        bbox_tar, _, mask_tar = self._crop_face_by_contour(img_tar, face_tar["contour"])
        ...
        img_tar = cv2.seamlessClone(
            patch_warp, img_tar, mask_tar_soft, center, cv2.NORMAL_CLONE
        )
        return img_tar
    ...
    def _get_face_info(pose_pred):
        ...
        if face_info["dataset"] == "coco":
            ...
            contour_ids = (
                list(range(33))[::-1] + list(range(33, 38)) + list(range(42, 47))
            )
            face_info["contour"] = keypoints[contour_ids]
        else:
            raise ValueError(
                "Can not obtain face landmark information"
        ...
        face_info["bbox"] = np.array(
            [
                face_info["contour"][:, 0].min(),
                face_info["contour"][:, 1].min(),
                face_info["contour"][:, 0].max(),
                face_info["contour"][:, 1].max(),
            ]
        ).astype(np.int64)
