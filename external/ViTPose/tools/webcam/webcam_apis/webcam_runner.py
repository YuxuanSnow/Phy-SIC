...
class WebcamRunner:
    ...
    def __init__(
        ...
        if buffer_sizes is None:
            buffer_sizes = {}
        # _frame_ buffer
        frame_buffer_size = buffer_sizes.get("_frame_", DEFAULT_FRAME_BUFFER_SIZE)
        self.buffer_manager.register_buffer("_frame_", frame_buffer_size)
        # _input_ buffer
        input_buffer_size = buffer_sizes.get("_input_", DEFAULT_INPUT_BUFFER_SIZE)
        self.buffer_manager.register_buffer("_input_", input_buffer_size)
        # _display_ buffer
        display_buffer_size = buffer_sizes.get("_display_", DEFAULT_DISPLAY_BUFFER_SIZE)
        ...
        for node_cfg in nodes:
            ...
            for buffer_info in node.registered_buffers:
                ...
                if buffer_name in self.buffer_manager:
                    continue
                buffer_size = buffer_sizes.get(buffer_name, DEFAULT_USER_BUFFER_SIZE)
                self.buffer_manager.register_buffer(buffer_name, buffer_size)
                logging.info(f"Register user buffer: {buffer_name}({buffer_size})")

            # Register events
            for event_info in node.registered_events:
                self.event_manager.register_event(
                    event_name=event_info.event_name, is_keyboard=event_info.is_keyboard
                )
