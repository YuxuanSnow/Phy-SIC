...
class InterHand3DDataset(Kpt3dSviewRgbImgTopDownDataset):
    ...
    def __init__(
        ...
        self.ann_info["heatmap3d_depth_bound"] = data_cfg["heatmap3d_depth_bound"]
    ...
    def _get_db(self):
        ...
        for img_id in self.img_ids:
            ...
            joint_cam = self._world2cam(
                joint_world.transpose(1, 0), camera_rot, camera_pos.reshape(3, 1)
            ).transpose(1, 0)
    ...
    def _report_metric(self, res_file, metrics):
        ...
        for pred, item in zip(preds, self.db):
            ...
            if "MRRPE" in metrics:
                if (
                    item["hand_type"].all()
                    and item["joints_3d_visible"][20, 0]
                    and item["joints_3d_visible"][41, 0]
                ):
                    rel_root_masks.append(True)

                    pred_left_root_img = np.array(
                        pred["keypoints"][41], dtype=np.float32
                    )[None, :]
                    pred_left_root_img[:, 2] += (
                        item["abs_depth"][0] + pred["rel_root_depth"]
                    )
                    ...
                    pred_right_root_img = np.array(
                        pred["keypoints"][20], dtype=np.float32
                    )[None, :]
                    ...
                    preds_rel_root.append(pred_left_root_cam - pred_right_root_cam)
                    gts_rel_root.append(
                        [item["joints_cam"][41] - item["joints_cam"][20]]
                    )
                else:
                    rel_root_masks.append(False)
                    preds_rel_root.append([[0.0, 0.0, 0.0]])
                    gts_rel_root.append([[0.0, 0.0, 0.0]])

            if "MPJPE" in metrics:
                pred_joint_coord_img = np.array(pred["keypoints"], dtype=np.float32)
                ...
                pred_joint_coord_cam = self._pixel2cam(
                    pred_joint_coord_img, item["focal"], item["princpt"]
                )
        ...
        preds_joint_coord_cam = np.array(preds_joint_coord_cam, dtype=np.float32)
        ...
        if "MRRPE" in metrics:
            info_str.append(
                ("MRRPE", keypoint_epe(preds_rel_root, gts_rel_root, rel_root_masks))
            )
        ...
        if "MPJPE" in metrics:
            info_str.append(
                (
                    "MPJPE_all",
                    keypoint_epe(preds_joint_coord_cam, gts_joint_coord_cam, all_masks),
                )
            )
            info_str.append(
                (
                    "MPJPE_single",
                    keypoint_epe(
                        preds_joint_coord_cam, gts_joint_coord_cam, single_masks
                    ),
                )
            )
            info_str.append(
                (
                    "MPJPE_interacting",
                    keypoint_epe(
                        preds_joint_coord_cam, gts_joint_coord_cam, interacting_masks
                    ),
                )
            )
        ...
        if "Handedness_acc" in metrics:
            info_str.append(
                (
                    "Handedness_acc",
                    self._get_accuracy(preds_hand_type, gts_hand_type, hand_type_masks),
                )
            )
