...
class HandCocoWholeBodyDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def __init__(
        ...
        if dataset_info is None:
            ...
            cfg = Config.fromfile("configs/_base_/datasets/coco_wholebody_hand.py")
        ...
        for img_id in self.img_ids:
            ...
            for obj in objs:
                for type in ["left", "right"]:
                    if obj[f"{type}hand_valid"] and max(obj[f"{type}hand_kpts"]) > 0:
                        joints_3d = np.zeros((num_joints, 3), dtype=np.float32)
                        joints_3d_visible = np.zeros((num_joints, 3), dtype=np.float32)

                        keypoints = np.array(obj[f"{type}hand_kpts"]).reshape(-1, 3)
