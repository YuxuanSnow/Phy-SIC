    ...
    def _get_db(self):
        ...
        for img_id in self.img_ids:
            ...
            camera_pos, camera_rot = (
                np.array(cameras[capture_id]["campos"][camera_name], dtype=np.float32),
                np.array(cameras[capture_id]["camrot"][camera_name], dtype=np.float32),
            )
            focal, principal_pt = (
                np.array(cameras[capture_id]["focal"][camera_name], dtype=np.float32),
                np.array(cameras[capture_id]["princpt"][camera_name], dtype=np.float32),
            )
            ...
            joint_valid = np.array(ann["joint_valid"], dtype=np.float32).reshape(2, -1)
