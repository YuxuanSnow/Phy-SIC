...
class TopDownMpiiTrbDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def _get_db(self, ann_file):
        ...
        imid2info = {int(osp.splitext(x["file_name"])[0]): x for x in data["images"]}
    ...
    def _report_metric(self, res_file):
        ...
        assert len(preds) == len(self.db), (
            f"len(preds)={len(preds)}, len(self.db)={len(self.db)}"
        )
        for pred, item in zip(preds, self.db):
            h, e = self._evaluate_kernel(
                pred["keypoints"],
                item["joints_3d"],
