...
class TopDownPoseTrack18VideoDataset(Kpt2dSviewRgbVidTopDownDataset):
    ...
    def __init__(
        ...
        self.frame_weight = (
            self.frame_weight_test if self.test_mode else self.frame_weight_train
        )
        ...
        self.frame_indices_test = data_cfg.get("frame_indices_test", [-2, -1, 0, 1, 2])
        ...
        for obj in objs:
            ...
            for index in indices:
                ...
                sup_image_file = cur_image_file.replace(
                    cur_image_name, str(support_idx).zfill(self.ph_fill_len) + ".jpg"
                )
    ...
    def _load_posetrack_person_detection_results(self):
        ...
        for det_res in all_boxes:
            ...
            bbox_id = bbox_id + 1
        print(f"=> Total boxes after filter low score@{self.det_bbox_thr}: {bbox_id}")
        return kpt_db
    ...
    def evaluate(self, results, res_folder=None, metric="mAP", **kwargs):
        ...
        for result in results:
            ...
            for i in range(batch_size):
                if not isinstance(image_paths[i], list):
                    ...
                    image_id = self.name2id[image_paths[i][0][len(self.img_prefix) :]]
        ...
        for image_id in kpts.keys():
            ...
            if self.use_nms:
                ...
                valid_kpts[image_id].append([img_kpts[_keep] for _keep in keep])
            else:
    ...
    def _do_keypoint_eval(self, gt_folder, pred_folder):
        ...
        if not has_poseval:
            raise ImportError(
                "Please install poseval package for evaluation"
