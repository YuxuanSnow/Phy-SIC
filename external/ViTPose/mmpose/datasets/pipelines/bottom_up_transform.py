...
def _get_multi_scale_size(image, input_size, current_scale, min_scale, use_udp=False):
    ...
    if w < h:
        ...
        h_resized = int(
            _ceil_to_multiples_of(min_input_w / w * h, 64) * current_scale / min_scale
        )
        if use_udp:
            scale_w = w - 1.0
        ...
        w_resized = int(
            _ceil_to_multiples_of(min_input_h / h * w, 64) * current_scale / min_scale
        )
        if use_udp:
            scale_h = h - 1.0
            ...
            for idx, pt in enumerate(p):
                if pt[2] > 0:
                    ...
                    if (
                        x < 0
                        or y < 0
                        or x >= self.output_size[0]
                        or y >= self.output_size[1]
                    ):
                        continue
                    ...
                    if self.use_udp:
                        ...
                        y0 = 3 * sigma + 1 + pt[1] - y
                        g = np.exp(
                            -((self.x - x0) ** 2 + (self.y - y0) ** 2) / (2 * sigma**2)
                        )
                    else:
                        g = self.g
...
class JointsEncoder:
    ...
    def __call__(self, joints):
        ...
        """
        visible_kpts = np.zeros(
            (self.max_num_people, self.num_joints, 2), dtype=np.float32
        )
        for i in range(len(joints)):
            ...
            for idx, pt in enumerate(joints[i]):
                ...
                if (
                    pt[2] > 0
                    and 0 <= y < self.output_size[1]
                    and 0 <= x < self.output_size[0]
                ):
                    if self.tag_per_joint:
                        visible_kpts[i][tot] = (
                            idx * self.output_size.prod() + y * self.output_size[0] + x,
                            1,
                        )
                    else:
...
class PAFGenerator:
    ...
    def _accumulate_paf_map_(self, pafs, src, dst, count):
        ...
        max_x = min(
            np.ceil(max(src[0], dst[0]) + self.limb_width), self.output_size[0] - 1
        )
        min_y = max(np.floor(min(src[1], dst[1]) - self.limb_width), 0)
        max_y = min(
            np.ceil(max(src[1], dst[1]) + self.limb_width), self.output_size[1] - 1
        )
    ...
    def __call__(self, joints):
        ...
        for idx, sk in enumerate(self.skeleton):
            count = np.zeros(
                (self.output_size[1], self.output_size[0]), dtype=np.float32
            )
...
class BottomUpRandomAffine:
    ...
    def __init__(
        self, rot_factor, scale_factor, scale_type, trans_factor, use_udp=False
    ):
    ...
    def __call__(self, results):
        ...
        if self.use_udp:
            for i, _output_size in enumerate(self.output_size):
                ...
                mask[i] = (
                    cv2.warpAffine(
                        ...
                        trans,
                        (int(_output_size[0]), int(_output_size[1])),
                        flags=cv2.INTER_LINEAR,
                    )
                    / 255
                )
                mask[i] = (mask[i] > 0.5).astype(np.float32)
                joints[i][:, :, 0:2] = warp_affine_joints(
            ...
            image = cv2.warpAffine(
                image, mat_input, (int(self.input_size[0]), int(self.input_size[1]))
            )
...
class BottomUpGenerateHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        heatmap_generator = self._generate(
            results["ann_info"]["num_joints"], results["ann_info"]["heatmap_size"]
        )
        target_list = list()
...
class BottomUpGenerateTarget:
    ...
    def __call__(self, results):
        """Generate multi-scale heatmap target for bottom-up."""
        heatmap_generator, joints_encoder = self._generate(
            results["ann_info"]["num_joints"], results["ann_info"]["heatmap_size"]
        )
        target_list = list()
...
class BottomUpGeneratePAFTarget:
    ...
    def __call__(self, results):
        ...
        paf_generator = self._generate(
            results["ann_info"]["heatmap_size"], self.skeleton
        )
        target_list = list()
        joints_list = results["joints"]
