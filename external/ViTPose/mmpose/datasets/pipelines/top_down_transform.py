...
class TopDownGenerateTarget:
    ...
    def __call__(self, results):
        ...
        if results["ann_info"].get("max_num_joints", None) is not None:
            ...
            padded_length = int(
                results["ann_info"].get("max_num_joints")
                - results["ann_info"].get("num_joints")
            )
            target_weight = np.concatenate(
                [target_weight, np.zeros((padded_length, 1), dtype=np.float32)], 0
            )
            target = np.concatenate(
                [target, np.zeros((padded_length, H, W), dtype=np.float32)], 0
            )
