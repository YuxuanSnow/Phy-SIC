...
class GetRootCenteredPose:
    ...
    def __init__(
        self, item, root_index, visible_item=None, remove_root=False, root_name=None
    ):
        self.item = item
    ...
    def __call__(self, results):
        ...
        if self.remove_root:
            results[self.item] = np.delete(results[self.item], root_idx, axis=-2)
...
class CollectCameraIntrinsics:
    ...
    def __call__(self, results):
        ...
        if self.need_distortion:
            ...
            intrinsics = np.concatenate(
                [intrinsics, camera_param["k"].reshape(3), camera_param["p"].reshape(2)]
            )
...
class RelativeJointRandomFlip:
    ...
    def __call__(self, results):
        if results.get(f"{self.item}_root_removed", False):
            raise RuntimeError(
                "The transform RelativeJointRandomFlip should "
        ...
        if np.random.rand() <= self.flip_prob:
            ...
            for i, item in enumerate(self.item):
                ...
                joints_flipped = fliplr_regression(
                    joints, flip_pairs, **self.flip_cfg[i]
                )
...
class Generate3DHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        target[idx[:, 0], idx[:, 1], idx[:, 2], idx[:, 3]] = local_target.reshape(-1)
...
class GenerateVoxel3DHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        target = np.zeros(
            (num_joints, cube_size[0], cube_size[1], cube_size[2]), dtype=np.float32
        )
