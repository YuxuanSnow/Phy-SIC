...
def _get_area(results):
    ...
    for result in results:
        if "bbox" in result:
            ...
            xmin = np.min(
                result["keypoints"][:, 0][result["keypoints"][:, 0] > 0], initial=1e10
            )
            xmax = np.max(result["keypoints"][:, 0])
            ymin = np.min(
                result["keypoints"][:, 1][result["keypoints"][:, 1] > 0], initial=1e10
            )
...
def get_track_id(
    ...
    for result in results:
        track_id, results_last, match_result = _track(
            result, results_last, tracking_thr
        )
        if track_id == -1:
        ...
        if use_one_euro:
            result["keypoints"] = _temporal_refine(result, match_result, fps=fps)
        del match_result

    return results, next_id


def vis_pose_tracking_result(
    model,
    img,
    result,
    radius=4,
