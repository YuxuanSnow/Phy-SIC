...
def _gather_pose_lifter_inputs(
    pose_results, bbox_center, bbox_scale, norm_pose_2d=False
):
...
def _collate_pose_sequence(pose_results, with_track_id=True, target_frame=-1):
    ...
    for idx in range(N):
        ...
        if not with_track_id:
            ...
            keypoints[target_frame] = pose_results[target_frame][idx]["keypoints"]
...
def inference_pose_lifter_model(
    ...
    pose_lifter_inputs = _gather_pose_lifter_inputs(
        pose_results_2d, bbox_center, bbox_scale, norm_pose_2d
    )
    pose_sequences_2d = _collate_pose_sequence(
        pose_lifter_inputs, with_track_id, target_idx
    )
    ...
    for seq in pose_sequences_2d:
        ...
        data = {
            ...
            "ann_info": {"num_joints": K, "flip_pairs": flip_pairs},
        }
    ...
    with torch.no_grad():
        result = model(
            input=batch_data["input"], metas=batch_data["metas"], return_loss=False
        )
...
def inference_interhand_3d_model(
    model,
    img_or_path,
    det_results,
    bbox_thr=None,
    ...
    with torch.no_grad():
        result = model(
            img=batch_data["img"], img_metas=batch_data["img_metas"], return_loss=False
        )
    ...
    pose_results = []
    for pose_3d, person_res, bbox_xyxy in zip(poses_3d, det_results, bboxes_xyxy):
...
def inference_mesh_model(
    ...
    for bbox in bboxes:
        ...
        data = {
            ...
            "bbox_score": bbox[4] if len(bbox) == 5 else 1,
            "dataset": dataset,
            "joints_2d": np.zeros((cfg.data_cfg.num_joints, 2), dtype=np.float32),
            "joints_2d_visible": np.zeros(
                (cfg.data_cfg.num_joints, 1), dtype=np.float32
            ),
            "joints_3d": np.zeros((cfg.data_cfg.num_joints, 3), dtype=np.float32),
            "joints_3d_visible": np.zeros(
                (cfg.data_cfg.num_joints, 3), dtype=np.float32
