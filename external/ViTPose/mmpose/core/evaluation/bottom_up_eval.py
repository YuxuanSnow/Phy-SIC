...
def split_ae_outputs(outputs, num_joints, with_heatmaps, with_ae, select_output_index):
...
def aggregate_scale(feature_maps_list, align_corners=False, aggregate_scale="average"):
...
def get_group_preds(grouped_joints, center, scale, heatmap_size, use_udp=False):
    ...
    if use_udp:
        if grouped_joints[0].shape[0] > 0:
            ...
            grouped_joints[0][..., :2] = warp_affine_joints(
                grouped_joints[0][..., :2], trans
            )
