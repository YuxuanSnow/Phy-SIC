...
class CuboidCenterHead(nn.Module):
    ...
    def __init__(
        self, space_size, space_center, cube_size, max_num=10, max_pool_kernel=3
    ):
    ...
    def _get_real_locations(self, indices):
        ...
        """
        real_locations = (
            indices.float() / (self.cube_size - 1) * self.grid_size
            + self.grid_center
            - self.grid_size / 2.0
        )
    ...
    def _get_3d_indices(indices, shape):
        ...
        indices_x = (indices // (shape[1] * shape[2])).reshape(
