...
class ParametricMesh(BasePose):
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if self.train_cfg["disc_step"] > 0 and self.with_gan:
            ...
            self.disc_step_count = (self.disc_step_count + 1) % self.train_cfg[
                "disc_step"
            ]
        ...
        # optimize generator
        pred_out = self.smpl(
            betas=pred_beta, body_pose=pred_pose[:, 1:], global_orient=pred_pose[:, :1]
        )
        ...
        gt_vertices = self.smpl(
            betas=gt_beta, body_pose=gt_pose[:, 3:], global_orient=gt_pose[:, :3]
        )["vertices"]
    ...
    def forward_test(
        self, img, img_metas, return_vertices=False, return_faces=False, **kwargs
    ):
    ...
    def get_3d_joints_from_mesh(self, vertices):
        ...
        return torch.matmul(self.joints_regressor.to(vertices.device), vertices)
    ...
    def show_result(
        ...
        **kwargs,
        ...
        for res in result:
            ...
            translation[:2] += (
                (camera_center - img_center[:, 0]) / focal_length * mean_depth
            )
