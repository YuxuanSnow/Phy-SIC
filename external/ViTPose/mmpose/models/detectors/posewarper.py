...
class PoseWarper(TopDown):
    ...
    def forward_train(self, imgs, target, target_weight, img_metas, **kwargs):
        ...
        assert num_frames == len(frame_weight), (
    ...
    def forward_dummy(self, img):
        ...
        assert batch_size > 1, "Input batch size to PoseWarper should be larger than 1."
        if batch_size == 2:
            warnings.warn(
                "Current batch size: 2, for pytorch2onnx and getting flops both."
            )
        else:
            warnings.warn(f"Current batch size: {batch_size}, for getting flops only.")
