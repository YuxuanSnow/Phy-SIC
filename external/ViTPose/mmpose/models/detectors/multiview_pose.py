...
class ProjectLayer(nn.Module):
    ...
    def get_voxel(self, feature_maps, meta, grid_size, grid_center, cube_size):
            ...
            if len(grid_center[0]) == 3 or grid_center[i][3] >= 0:
                ...
                for c in range(n):
                    ...
                    trans = torch.as_tensor(
                        get_affine_transform(center, scale / 200.0, 0, self.image_size),
                        dtype=torch.float,
                        device=device,
                    )
        ...
        cubes = cubes.view(
            batch_size, num_channels, cube_size[0], cube_size[1], cube_size[2]
        )
        return cubes, grids

    def forward(self, feature_maps, meta, grid_size, grid_center, cube_size):
        cubes, grids = self.get_voxel(
            feature_maps, meta, grid_size, grid_center, cube_size
        )
        return cubes, grids
...
class DetectAndRegress(BasePose):
        ...
        **kwargs,
        ...
        if return_loss:
            return self.forward_train(
                img, img_metas, targets, masks, targets_3d, input_heatmaps
            )
        else:
    ...
    def forward_train(
        ...
        if not self.freeze_2d:
            ...
            losses_2d_ = self.backbone.get_loss(
...
class VoxelSinglePose(BasePose):
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        pred = human_candidates.new_zeros(
            batch_size, num_candidates, self.num_joints, 5
        )
        ...
        for n in range(num_candidates):
            ...
            num_valid = index.sum()
            if num_valid > 0:
                pose_input_cube, coordinates = self.project_layer(
                    feature_maps,
                    img_metas,
                    self.sub_space_size,
                ...
                valid_weights.append(
                    gt_3d_vis[index, pred[index, n, 0, 3].long(), :, 0:1].float()
                )
        ...
        if len(valid_preds) > 0:
            ...
            losses.update(
                self.pose_head.get_loss(valid_preds, valid_targets, valid_weights)
            )
        else:
            ...
            coordinates = (
                feature_maps[0]
                .new_zeros(batch_size, *self.sub_cube_size, 3)
                .view(batch_size, -1, 3)
            )
            ...
            losses.update(
                self.pose_head.get_loss(pose_3d, pseudo_targets, pseudo_weights)
            )
        if return_preds:
            return pred, losses
        else:
    ...
    def forward_test(
        self, img, img_metas, feature_maps=None, human_candidates=None, **kwargs
    ):
        """Defines the computation performed at training.
        Note:
            batch_size: N
    ...
    def forward_dummy(self, feature_maps, num_candidates=5):
        ...
        pose_input_cube = feature_maps[0].new_zeros(
            batch_size, num_channels, *self.sub_cube_size
        )
        for n in range(num_candidates):
...
class VoxelCenterDetector(BasePose):
    ...
    def forward(
        self, img, img_metas, return_loss=True, feature_maps=None, targets_3d=None
    ):
        """
        Note:
            batch_size: N
    ...
    def forward_train(
        self, img, img_metas, feature_maps=None, targets_3d=None, return_preds=False
    ):
        """
        Note:
            batch_size: N
        ...
        center_candidates = self.assign2gt(
            center_candidates, gt_centers, gt_num_persons
        )
    ...
    def forward_test(self, img, img_metas, feature_maps=None):
        ...
        center_candidates = self.center_head(center_heatmaps_3d)
        center_candidates[..., 3] = (
            center_candidates[..., 4] > self.test_cfg["center_threshold"]
        ).float() - 1.0
    ...
    def forward_dummy(self, feature_maps):
        ...
        initial_cubes = feature_maps[0].new_zeros(
            batch_size, num_channels, *self.cube_size
        )
