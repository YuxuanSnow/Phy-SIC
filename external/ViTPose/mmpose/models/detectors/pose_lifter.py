...
class PoseLifter(BasePose):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        if return_loss:
            return self.forward_train(input, target, target_weight, metas, **kwargs)
        else:
    ...
    def forward_train(self, input, target, target_weight, metas, **kwargs):
        ...
        if self.with_traj:
            ...
            traj_losses = self.traj_head.get_loss(
                traj_output, kwargs["traj_target"], None
            )
    ...
    @deprecated_api_warning(
        {"pose_limb_color": "pose_link_color"}, cls_name="PoseLifter"
    )
    def show_result(
        self,
        result,
