...
class UpsampleUnit(nn.Module):
    ...
    def forward(self, x, up_x):
        ...
        if self.ind > 0:
            up_x = F.interpolate(
                up_x, size=(x.size(2), x.size(3)), mode="bilinear", align_corners=True
            )
...
class SingleStageNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.downsample = DownsampleModule(
            Bottleneck, num_blocks, num_units, has_skip, norm_cfg, in_channels
        )
        self.upsample = UpsampleModule(
            unit_channels, num_units, gen_skip, gen_cross_conv, norm_cfg, in_channels
        )
                ...
                if k.startswith("layer"):
                    if "downsample.0" in k:
                        state_dict["bottlenecks"][
                            k.replace("downsample.0", "downsample.conv")
                        ] = v
                    elif "downsample.1" in k:
                        state_dict["bottlenecks"][
                            k.replace("downsample.1", "downsample.bn")
                        ] = v
                    else:
