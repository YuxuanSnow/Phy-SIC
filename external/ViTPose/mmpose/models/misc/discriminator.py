...
class BaseDiscriminator(nn.Module):
    ...
    def _check(self):
        ...
        if not isinstance(self.use_activation, tuple):
            raise TypeError(
                f"use_activation require tuple, get {type(self.use_activation)}"
            )
    ...
    def create_layers(self):
        ...
        for i in range(l_fc_layer - 1):
            self.fc_blocks.add_module(
                ...
                module=nn.Linear(
                    in_features=self.fc_layers[i], out_features=self.fc_layers[i + 1]
                ),
            )
...
class ShapeDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, fc_layers, use_dropout, drop_prob, use_activation):
        if fc_layers[-1] != 1:
            msg = (
                f"the neuron count of the last layer must be 1, but got {fc_layers[-1]}"
            )
            raise ValueError(msg)
...
class PoseDiscriminator(nn.Module):
    ...
    def __init__(self, channels, joint_count):
        ...
        if channels[-1] != 1:
            msg = (
                f"the neuron count of the last layer must be 1, but got {channels[-1]}"
            )
            raise ValueError(msg)
        ...
        for idx in range(joint_count):
            self.fc_layer.append(
                nn.Linear(in_features=channels[len_channels - 2], out_features=1)
            )
...
class SMPLDiscriminator(nn.Module):
    ...
    def _create_sub_modules(self):
        ...
        self.pose_discriminator = PoseDiscriminator(
            self.per_joint_channel, self.joint_count
        )
    ...
    def forward(self, thetas):
        ...
        if poses.dim() == 2:
            ...
            rotate_matrixs = (
                poses.contiguous().view(batch_size, 24, 9)[:, 1:, :].contiguous()
            )
