...
class PoseWarperNeck(nn.Module):
    ...
    def __init__(
        ...
        assert self.num_offset_layers > 0, (
            "Number of offset layers should be larger than 0."
        )
        ...
        if has_mmcv_full:
            ...
            raise ImportError(
                "Please install the full version of mmcv to use `DeformConv2d`."
            )
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                for k in range(m.weight.size(0)):
                    filler[
                        k, k, int(m.weight.size(2) / 2), int(m.weight.size(3) / 2)
                    ] = 1.0
        ...
        if len(inputs) > 1:
            ...
            for i in range(num_frames):
                if frame_weight[i] == 0:
                    continue
                output_heatmap += (
                    warped_heatmap[i * batch_size : (i + 1) * batch_size]
                    * frame_weight[i]
                )
