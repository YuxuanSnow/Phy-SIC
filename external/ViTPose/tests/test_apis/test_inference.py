...
def test_top_down_demo():
    ...
    pose_results, _ = inference_top_down_pose_model(
        pose_model, image_name, person_result, format="xywh", dataset_info=dataset_info
    )
    # show the results
    vis_pose_result(pose_model, image_name, pose_results, dataset_info=dataset_info)

    # AIC demo
    pose_model = init_pose_model(
        "configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/aic/res50_aic_256x192.py",
        None,
        device="cpu",
    )
    ...
    # build the pose model from a config file and a checkpoint file
    pose_model = init_pose_model(
        "configs/face/2d_kpt_sview_rgb_img/topdown_heatmap/300w/res50_300w_256x256.py",
        None,
        device="cpu",
    )
    ...
    # build the pose model from a config file and a checkpoint file
    pose_model = init_pose_model(
        "configs/face/2d_kpt_sview_rgb_img/topdown_heatmap/aflw/res50_aflw_256x256.py",
        None,
        device="cpu",
    )
    ...
    # build the pose model from a config file and a checkpoint file
    pose_model = init_pose_model(
        "configs/face/2d_kpt_sview_rgb_img/topdown_heatmap/cofw/res50_cofw_256x256.py",
        None,
        device="cpu",
    )
...
def test_process_mmdet_results():
    ...
    _ = process_mmdet_results(mmdet_results=(det_results, det_mask_results), cat_id=1)
