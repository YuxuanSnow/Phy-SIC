...
def test_build_optimizers():
    ...
    optimizer_cfg = dict(
        model1=dict(type="SGD", lr=base_lr, weight_decay=base_wd, momentum=momentum),
        model2=dict(type="SGD", lr=base_lr, weight_decay=base_wd, momentum=momentum),
    )
    model = ExampleModel()
    ...
    for i in range(2):
        ...
        assert torch.equal(
            param_groups["params"][0], param_dict[f"model{i + 1}.weight"]
        )
        assert torch.equal(param_groups["params"][1], param_dict[f"model{i + 1}.bias"])
    ...
    for i in range(2):
        ...
        assert len(param_groups["params"]) == 2
        assert torch.equal(
            param_groups["params"][0], param_dict[f"module.model{i + 1}.weight"]
        )
        assert torch.equal(
            param_groups["params"][1], param_dict[f"module.model{i + 1}.bias"]
        )
    ...
    assert optimizer.defaults["weight_decay"] == base_wd
    assert len(param_groups["params"]) == 4
    assert torch.equal(param_groups["params"][0], param_dict["module.model1.weight"])
    assert torch.equal(param_groups["params"][1], param_dict["module.model1.bias"])
    assert torch.equal(param_groups["params"][2], param_dict["module.model2.weight"])
    assert torch.equal(param_groups["params"][3], param_dict["module.model2.bias"])
