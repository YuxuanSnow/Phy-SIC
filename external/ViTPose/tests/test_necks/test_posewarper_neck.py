...
def test_posewarper_neck():
    ...
    with pytest.raises(AssertionError):
        ...
        _ = PoseWarperNeck(
            out_channels=3, in_channels=512, inner_channels=128, trans_conv_kernel=2
        )

    with pytest.raises(TypeError):
        # test type of res_blocks_cfg
        _ = PoseWarperNeck(
            out_channels=3, in_channels=512, inner_channels=128, res_blocks_cfg=2
        )
...
def _demo_inputs(input_shape=(80, 48, 4, 4), length=1):
    ...
    """
    imgs = [torch.FloatTensor(np.random.random(input_shape)) for _ in range(length)]
    return imgs
