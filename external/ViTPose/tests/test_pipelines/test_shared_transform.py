...
def test_compose():
    ...
    test_pipeline = [
        dict(
            type="Collect", keys=["img", ("img", "img_rename")], meta_keys=["img_file"]
        )
    ]
...
def test_albu_transform():
    ...
    albu_transform = dict(
        type="Albumentation",
        transforms=[dict(type="RandomBrightnessContrast", p=0.2), dict(type="ToFloat")],
    )
...
def test_rename_keys():
    ...
    pipeline = dict(
        type="RenameKeys",
        key_pairs=[("joints_3d", "target"), ("joints_3d_visible", "target_weight")],
    )
    pipeline = build_from_cfg(pipeline, PIPELINES)
    results = pipeline(results)
