...
from mmpose.datasets.pipelines import (
...
def _get_joints(anno, ann_info, int_sigma):
    ...
    if ann_info["scale_aware_sigma"]:
        joints = np.zeros((num_people, ann_info["num_joints"], 4), dtype=np.float32)
    else:
        joints = np.zeros((num_people, ann_info["num_joints"], 3), dtype=np.float32)

    for i, obj in enumerate(anno):
        joints[i, : ann_info["num_joints"], :3] = np.array(obj["keypoints"]).reshape(
            [-1, 3]
        )
        if ann_info["scale_aware_sigma"]:
            # get person box
...
def test_bottomup_pipeline():
    ...
    ann_info["flip_index"] = [0, 2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15]
    ...
    # test HorizontalFlip
    random_horizontal_flip = BottomUpRandomFlip(flip_prob=1.0)
    ...
    random_horizontal_flip = BottomUpRandomFlip(flip_prob=0.0)
    results_horizontal_flip = random_horizontal_flip(copy.deepcopy(results))
    ...
    with pytest.raises(AssertionError):
        results_horizontal_flip = random_horizontal_flip(copy.deepcopy(results_copy))
    ...
    random_affine_transform = BottomUpRandomAffine(30, [0.75, 1.5], "short", 40)
    results_affine_transform = random_affine_transform(copy.deepcopy(results))
    assert results_affine_transform["img"].shape == (512, 384, 3)
    ...
    results_copy["mask"] = mask_list[0]
    with pytest.raises(AssertionError):
        results_horizontal_flip = random_affine_transform(copy.deepcopy(results_copy))
    ...
    with pytest.raises(ValueError):
        random_affine_transform = BottomUpRandomAffine(
            30, [0.75, 1.5], "short-long", 40
        )
        results_affine_transform = random_affine_transform(copy.deepcopy(results))

    # test BottomUpGenerateTarget
    generate_multi_target = BottomUpGenerateTarget(2, 30)
    results_generate_multi_target = generate_multi_target(copy.deepcopy(results))
    assert "targets" in results_generate_multi_target
    assert (
        len(results_generate_multi_target["targets"])
...
def test_BottomUpGenerateHeatmapTarget():
    ...
    assert (
        len(results_generate_heatmap_target["target"])
        == results["ann_info"]["num_scales"]
    )
...
def test_BottomUpGeneratePAFTarget():
    ...
    sqrt = np.sqrt(2) / 2
    assert (
        results_generate_paf_target["target"]
        == np.array(
            [
                [
