...
def _check_rot90(origin_imgs, result_imgs):
    if (
        origin_imgs.shape[0] == result_imgs.shape[1]
        and origin_imgs.shape[1] == result_imgs.shape[0]
    ):
        return True
    else:
        return False
...
def _load_test_data():
    ...
    ann_info["joint_weights"] = np.ones(
        ann_info["num_joints"], dtype=np.float32
    ...
    results["rotation"] = 0
    results["image_file"] = os.path.join(img_prefix, ann_data["imgname"][index])
    ...
    if "iuv_names" in ann_data.keys():
        results["iuv_file"] = os.path.join(iuv_prefix, ann_data["iuv_names"][index])
...
def test_mesh_pipeline():
    ...
    # test flip
    random_flip = MeshRandomFlip(flip_prob=1.0)
    ...
    _check_normalize(
        results_tensor["img"].data.numpy(),
        results_normalize["img"].data.numpy(),
        norm_cfg,
    )
