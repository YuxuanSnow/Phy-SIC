...
def test_top_down_pipeline():
    ...
    # test flip
    random_flip = TopDownRandomFlip(flip_prob=1.0)
    ...
    halfbody_transform = TopDownHalfBodyTransform(
        num_joints_half_body=8, prob_half_body=1.0
    )
    ...
    assert results_target["target"].shape == (
        num_joints,
        results["ann_info"]["heatmap_size"][1],
    ...
    generate_target = TopDownGenerateTarget(sigma=[2, 3], unbiased_encoding=False)
    results_target = generate_target(copy.deepcopy(results_tensor))
    assert "target" in results_target
    assert results_target["target"].shape == (
        2,
