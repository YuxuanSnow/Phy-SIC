...
def test_joint_transforms():
    ...
    pipeline = [
        ...
        dict(type="NormalizeJointCoordinate", item="target", mean=mean, std=std),
        dict(type="PoseSequenceToTensor", item="target"),
        dict(type="ImageCoordinateNormalization", item="input_2d", norm_camera=True),
    ...
    _joints_0_flipped[..., 0] = 2 * joints_0[..., 0:1, 0] - _joints_0_flipped[..., 0]
    ...
    # manually do transformations
    center = np.array(
        [0.5 * results["image_width"], 0.5 * results["image_height"]], dtype=np.float32
    )
    ...
    camera_param_0["c"] = (camera_param_0["c"] - np.array(center)[:, None]) / scale
    camera_param_0["f"] = camera_param_0["f"] / scale
    np.testing.assert_array_almost_equal(camera_param_0["c"], camera_param_1["c"])
    np.testing.assert_array_almost_equal(camera_param_0["f"], camera_param_1["f"])
...
def test_camera_projection():
    ...
    pipeline_2 = [
        ...
        dict(
            type="Collect", keys=["input_3d_wp", "input_3d_p", "input_2d"], meta_keys=[]
        ),
    ]
    ...
    np.testing.assert_allclose(output1["input_3d_wp"], output1["input_3d_p"], rtol=1e-6)

    np.testing.assert_allclose(output2["input_3d_wp"], output2["input_3d_p"], rtol=1e-6)
...
def test_3d_heatmap_generation():
    ...
    # test joint_indices
    pipeline = Compose([dict(type="Generate3DHeatmapTarget", joint_indices=[0, 8, 16])])
    results_3d = pipeline(results)
