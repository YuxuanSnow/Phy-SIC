...
def test_temporal_regression_head():
    ...
    assert torch.allclose(loss["reg_loss"], torch.tensor(0.0))
    ...
    assert acc["mpjpe"] == 0.0
    np.testing.assert_almost_equal(acc["p_mpjpe"], 0.0, decimal=6)
    ...
    assert acc["mpjpe"] == 0.0
    np.testing.assert_almost_equal(acc["p_mpjpe"], 0.0)
    ...
    assert torch.allclose(loss["traj_loss"], torch.tensor(0.0))
