...
def test_vipnas_forward():
    ...
    # Test forward train
    losses = detector.forward(imgs, target, target_weight, img_metas, return_loss=True)
...
def test_topdown_forward():
    ...
    model_cfg = dict(
        ...
        keypoint_head=dict(
            ...
            loss_keypoint=[
                dict(type="JointsMSELoss", use_target_weight=True, loss_weight=1.0)
            ],
        ),
        train_cfg=dict(),
        test_cfg=dict(
    ...
    model_cfg = dict(
        ...
        keypoint_head=dict(
            ...
            use_prm=False,
            norm_cfg=dict(type="BN"),
            loss_keypoint=[dict(type="JointsMSELoss", use_target_weight=True)] * 3
