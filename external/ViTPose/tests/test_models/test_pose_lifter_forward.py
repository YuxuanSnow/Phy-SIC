...
def _create_inputs(
    ...
    target = np.zeros((batch_size, joint_num_out, joint_channel_out), dtype=np.float32)
    target_weight = np.ones(
        (batch_size, joint_num_out, joint_channel_out), dtype=np.float32
    )

    meta_info = {
        "root_position": np.zeros((1, joint_channel_out), np.float32),
        "root_position_index": 0,
        "target_mean": np.zeros((joint_num_out, joint_channel_out), np.float32),
    ...
    if semi:
        ...
        unlabeled_pose_in = rng.rand(
            batch_size, joint_num_in * joint_channel_in, seq_len
        )
        ...
        inputs["unlabeled_input"] = torch.FloatTensor(unlabeled_pose_in).requires_grad_(
            True
        )
