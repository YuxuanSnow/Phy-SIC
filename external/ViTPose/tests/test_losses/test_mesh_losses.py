...
def test_mesh_loss():
    ...
    assert torch.allclose(losses["vertex_loss"], torch.tensor(0.0))
    assert torch.allclose(losses["smpl_pose_loss"], torch.tensor(0.0))
    assert torch.allclose(losses["smpl_beta_loss"], torch.tensor(0.0))
    assert torch.allclose(losses["joints_3d_loss"], torch.tensor(0.0))
    assert torch.allclose(losses["joints_2d_loss"], torch.tensor(0.0))
    ...
    losses = loss(fake_pred, fake_gt)
    assert torch.allclose(losses["vertex_loss"], torch.tensor(1.0))
    assert torch.allclose(losses["smpl_pose_loss"], torch.tensor(1.0))
    assert torch.allclose(losses["smpl_beta_loss"], torch.tensor(1.0))
