...
def test_multi_loss_factory():
    ...
    fake_label = torch.zeros((1, 3, 64, 64))
    fake_mask = torch.zeros((1, 64, 64))
    assert torch.allclose(loss(fake_pred, fake_label, fake_mask), torch.tensor(0.0))
    ...
    assert torch.allclose(loss(fake_pred, fake_label, fake_mask), torch.tensor(1.0))
    ...
    loss = build_loss(loss_cfg)
    assert torch.allclose(loss(fake_tags, fake_joints)[0], torch.tensor(0.0))
    assert torch.allclose(loss(fake_tags, fake_joints)[1], torch.tensor(0.0))

    fake_tags[0, 0, 0] = 1.0
    fake_tags[0, 10, 0] = 0.0
    ...
    heatmaps_losses, push_losses, pull_losses = loss(
        fake_outputs, fake_heatmaps, fake_masks, fake_joints
    )
