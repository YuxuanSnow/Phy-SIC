...
def test_bce_loss():
    ...
    fake_label = torch.zeros((1, 2))
    assert torch.allclose(loss(fake_pred, fake_label), -torch.log(torch.tensor(0.5)))
    ...
    assert torch.allclose(
        loss(fake_pred, fake_label, fake_weight), -torch.log(torch.tensor(0.5))
    )

    fake_weight[:, 0] = 0
    assert torch.allclose(
        loss(fake_pred, fake_label, fake_weight), -0.5 * torch.log(torch.tensor(0.5))
    )
