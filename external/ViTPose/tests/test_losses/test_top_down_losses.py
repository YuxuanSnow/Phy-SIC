...
def test_adaptive_wing_loss():
    ...
    assert torch.allclose(loss(fake_pred, fake_label, None), torch.tensor(0.0))
    ...
    assert torch.allclose(
        loss(fake_pred, fake_label, torch.ones([1, 3, 1])), torch.tensor(0.0)
    )
...
def test_mse_loss():
    ...
    fake_pred = torch.ones((1, 3, 64, 64))
    fake_label = torch.zeros((1, 3, 64, 64))
    assert torch.allclose(loss(fake_pred, fake_label, None), torch.tensor(1.0))
    ...
    assert torch.allclose(loss(fake_pred, fake_label, target_weight), torch.tensor(0.5))
    ...
    assert torch.allclose(loss(fake_pred, fake_label, target_weight), torch.tensor(0.0))
