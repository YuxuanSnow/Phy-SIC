...
model = dict(
    ...
    keypoint_head=dict(
        ...
        norm_cfg=dict(type="BN"),
        loss_keypoint=(
            [dict(type="JointsMSELoss", use_target_weight=True, loss_weight=0.25)] * 3
            + [dict(type="JointsOHKMMSELoss", use_target_weight=True, loss_weight=1.0)]
        )
        * 3,
    ),
    train_cfg=dict(),
    test_cfg=dict(
        flip_test=True, post_process="megvii", shift_heatmap=False, modulate_kernel=5
    ),
)
