...
model = dict(
    ...
    ),  # dataset_size // samples_per_gpu // gpu_num * warmup_epochs
    train_cfg=dict(),
...
val_data_cfg = dict(
    ...
    temporal_padding=True,
    joint_2d_src="detection",
    joint_2d_det_file=f"{data_root}/joint_2d_det_files/" + "cpn_ft_h36m_dbb_test.npy",
    need_camera_param=True,
    camera_param_file=f"{data_root}/annotation_body3d/cameras.pkl",
)
...
train_labeled_pipeline = [
    ...
    dict(
        ...
        flip_cfg=[
            dict(center_mode="static", center_x=0.0),
    ...
    dict(
        type="Collect",
        keys=[("input_2d", "input"), "target", ("root_position", "traj_target")],
        meta_name="metas",
...
train_unlabeled_pipeline = [
    ...
    dict(
        type="Collect",
        keys=[
            ("input_2d", "unlabeled_input"),
            ("target_2d", "unlabeled_target_2d"),
            "intrinsics",
        ],
