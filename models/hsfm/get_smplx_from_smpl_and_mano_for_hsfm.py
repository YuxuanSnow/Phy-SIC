...
smplx_right_hand_joint_idx_reordered = [
    ORIGINAL_SMPLX_JOINT_NAMES.index(x) for x in MANO_RIGHT_REORDER_KEYPOINTS
]
...
# the last 5 joints are presumably finger tips
smplx_left_hand_joint_idx_reordered = [
    ORIGINAL_SMPLX_JOINT_NAMES.index(x) for x in MANO_LEFT_REORDER_KEYPOINTS
]
...
class ShapeConverter:
    def __init__(
        ...
        assert self.inbm_type in ["smil", "smpl", "smpla", "smplh"], (
            "Only SMPL to SMPL-X conversion is supported"
        )
        assert self.outbm_type in ["smplx", "smplxa"], (
            "Only SMPL to SMPL-X conversion is supported"
        )

        self.smpltosmplx = self.load_smpltosmplx()
        self.inbm, self.inshapedirs = self.load_body_model(
            model_type=self.inbm_type, num_betas=self.in_num_betas
        )
    ...
    def load_smpltosmplx(self):
        smpl_to_smplx_path = osp.join(
            self.essentials_folder, "body_models/smpl_to_smplx.pkl"
        )
    ...
    def load_body_model(self, model_type, num_betas=None):
        if model_type in ["smpl", "smplx"]:
            ...
            model_path = osp.join(
                self.essentials_folder, "body_models/smpla/SMPLA_NEUTRAL.pth"
            )
            bm = torch.load(model_path)
            shapedirs = bm["smpla_shapedirs"]
        elif model_type == "smil":
            model_path = osp.join(
                self.essentials_folder, "body_models/smil/smil_packed_info.pth"
            )
            bm = torch.load(model_path)
            shapedirs = bm["shapedirs"]
            ...
            model_path = osp.join(
                self.essentials_folder, "body_models/smplh/neutral/model.npz"
            )
    ...
    def forward(self, in_betas):
        ...
        assert in_betas.shape[1] == self.inshapedirs.shape[-1], (
            "Input betas do not match input body model"
        )
        ...
        in_shape_displacement = torch.einsum(
            "nm,bmv->bnv", self.smpltosmplx, in_shape_displacement
        )
        ...
        lsq_arr = torch.matmul(
            torch.inverse(torch.matmul(out_shapedirs.t(), out_shapedirs)),
            out_shapedirs.t(),
        )
...
def main(
    ...
    output_dir = os.path.join(output_dir, os.path.basename(mano_dir))
    ...
    assert len(mano_files) == len(smpl_files), (
        f"Number of mano files ({len(mano_files)}) and smpl files ({len(smpl_files)}) do not match"
    )

    smpl_to_smplx_shape_converter = ShapeConverter(
        essentials_folder="./",
        inbm_type="smpl",
        outbm_type="smplx",
        def_outbm=True,
    ...
    for mano_file, smpl_file in tqdm(zip(mano_files, smpl_files)):
        ...
        assert (
            frame_idx == os.path.basename(smpl_file).split(".")[0].split("_params_")[1]
        ), f"Frame index mismatch between {mano_file} and {smpl_file}"
        ...
        for person_id in mano_data.keys():
            ...
            if right_hand_mano_data is not None:
                converted_right_hand_pose = conv_mano_to_smplx_hand_pose(
                    right_hand_mano_data["hand_pose"], 1.0
                )
            else:
                converted_right_hand_pose = None

            if left_hand_mano_data is not None:
                converted_left_hand_pose = conv_mano_to_smplx_hand_pose(
                    left_hand_mano_data["hand_pose"], 0.0
                )
            else:
                converted_left_hand_pose = None

            converted_betas = conv_body_shape(
                smpl_params["betas"], smpl_to_smplx_shape_converter
            )
            converted_body_pose = conv_smpl_to_smplx_body_pose(
                smpl_params["body_pose"]
            )  # (21, 3, 3)
            converted_global_orient = smpl_params["global_orient"]

            # convert 3by3 rotation matrix to axis angle
            converted_global_orient_axis_angle = rotation_matrix_to_angle_axis(
                converted_global_orient
            )  # (1, 3)
            converted_body_pose_axis_angle = rotation_matrix_to_angle_axis(
                converted_body_pose
            )  # (21, 3)
            ...
            optimizing_params = {}
            global_orient = (
                torch.from_numpy(smplx_params["global_orient"])
                .to(device)
                .float()
                .reshape(1, -1)
            )
            body_pose = (
                torch.from_numpy(smplx_params["body_pose"])
                .to(device)
                .float()
                .reshape(1, -1)
            )  # (1, 21*3) 21 joints except the first pevlis joint
            betas = (
                torch.from_numpy(smplx_params["betas"])
                .to(device)
                .float()
                .reshape(1, -1)
            )
            if smplx_params["left_hand_pose"] is not None:
                left_hand_pose = (
                    torch.from_numpy(smplx_params["left_hand_pose"])
                    .to(device)
                    .float()
                    .reshape(1, -1)
                )
                # left wrist pose is the 19th joint in the body pose
                optimizing_params["left_wrist_pose"] = nn.Parameter(
                    body_pose[0:1, 19 * 3 : (19 + 1) * 3]
                )
            else:
                left_hand_pose = None
            ...
            if smplx_params["right_hand_pose"] is not None:
                right_hand_pose = (
                    torch.from_numpy(smplx_params["right_hand_pose"])
                    .to(device)
                    .float()
                    .reshape(1, -1)
                )
                # right wrist pose is the 20th joint in the body pose
                optimizing_params["right_wrist_pose"] = nn.Parameter(
                    body_pose[0:1, 20 * 3 : (20 + 1) * 3]
                )
            else:
            ...
            if vis:
                ...
                mesh = trimesh.Trimesh(
                    vertices.copy(),
                    smplx_layer.faces.copy(),
                    vertex_colors=vertex_colors,
                )

                saving_path = os.path.join(
                    vis_output_dir,
                    f"{frame_idx}_smplx_person_{person_id:02d}_original.obj",
                )
            ...
            if len(optimizing_params) > 0:
                ...
                if left_hand_pose is not None:
                    mano_left_hand_joints = (
                        torch.from_numpy(
                            mano_data[person_id]["left_hand"]["pred_keypoints_3d"]
                        )
                        .to(device)
                        .float()
                        .detach()
                    ...
                    mano_left_hand_joints = (
                        mano_left_hand_joints - mano_left_hand_joints[0:1, :]
                    )
                    body_pose = torch.cat(
                        [
                            body_pose[:, : 19 * 3],
                            optimizing_params["left_wrist_pose"],
                            body_pose[:, (19 + 1) * 3 :],
                        ],
                        dim=1,
                    )
                if right_hand_pose is not None:
                    mano_right_hand_joints = (
                        torch.from_numpy(
                            mano_data[person_id]["right_hand"]["pred_keypoints_3d"]
                        )
                        .to(device)
                        .float()
                        .detach()
                    ...
                    mano_right_hand_joints = (
                        mano_right_hand_joints - mano_right_hand_joints[0:1, :]
                    )
                    body_pose = torch.cat(
                        [
                            body_pose[:, : 20 * 3],
                            optimizing_params["right_wrist_pose"],
                            body_pose[:, (20 + 1) * 3 :],
                        ],
                        dim=1,
                    )
                ...
                def create_closure():
                    def closure():
                        ...
                        smplx_right_hand_joints = smplx_joints[
                            smplx_right_hand_joint_idx_reordered, :
                        ]  # (21, 3)
                        smplx_left_hand_joints = smplx_joints[
                            smplx_left_hand_joint_idx_reordered, :
                        ]  # (21, 3)
                        ...
                        if right_hand_pose is not None:
                            ...
                            smplx_right_hand_joints = (
                                smplx_right_hand_joints
                                - smplx_right_hand_joints[0:1, :]
                            )
                            loss += (
                                (smplx_right_hand_joints - mano_right_hand_joints)
                                .pow(2)
                                .sum()
                            )
                        if left_hand_pose is not None:
                            smplx_left_hand_joints = (
                                smplx_left_hand_joints - smplx_left_hand_joints[0:1, :]
                            )
                            loss += (
                                (smplx_left_hand_joints - mano_left_hand_joints)
                                .pow(2)
                                .sum()
                            )
            ...
            if vis:
                ...
                saving_path = os.path.join(
                    vis_output_dir, f"{frame_idx}_smplx_person_{person_id:02d}.obj"
                )
                mesh.export(saving_path)

                print(f"Saved to {saving_path}")

        with open(os.path.join(output_dir, f"smplx_params_{frame_idx}.pkl"), "wb") as f:
            pickle.dump(result_dict, f)
        print(f"Saved smplx_params_{frame_idx}.pkl to {output_dir}")
