...
class FluxControlRemovalPipeline(
    DiffusionPipeline,
    FluxLoraLoaderMixin,
    FromSingleFileMixin,
    TextualInversionLoaderMixin,
    ...
    def prepare_image_with_mask(
        ...
        foreground_image_latents = self.vae.encode(
            foreground_images.to(self.vae.dtype)
        ).latent_dist.sample()
        ...
        masks = torch.nn.functional.interpolate(
            masks,
            size=(
                masks.shape[2] // self.vae_scale_factor,
                masks.shape[3] // self.vae_scale_factor,
            ),
        ).to(dtype)
        masks = 1 - masks
        ...
        control_image = torch.cat(
            [masked_image_latents, foreground_image_latents, masks], dim=1
        )  # bs, 16+16+16=48, h, w
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                # print(latents.shape) # 64
                # print(control_image.shape) # 192
                latent_model_input = torch.cat([latents, control_image], dim=2)
                # print(latent_model_input.shape) # 276
